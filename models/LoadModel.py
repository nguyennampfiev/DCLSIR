...
class MainModelSmall(nn.Module):
    def __init__(self, config):
        super(MainModelSmall, self).__init__()
        self.use_dcl = config.use_dcl
        self.num_classes = config.numcls
        self.backbone_arch = config.backbone
        self.use_Asoftmax = config.use_Asoftmax
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(output_size=1)
        self.reduce_chanel = nn.Conv2d(in_channels=2048, out_channels=256, kernel_size=1, stride=1, padding=0)
        self.classifier = nn.Linear(256, self.num_classes, bias=False)

        if self.use_dcl:
            if config.cls_2:
                self.classifier_swap = nn.Linear(256, 2, bias=False)
            if config.cls_2xmul:
                self.classifier_swap = nn.Linear(256, 2*self.num_classes, bias=False)
